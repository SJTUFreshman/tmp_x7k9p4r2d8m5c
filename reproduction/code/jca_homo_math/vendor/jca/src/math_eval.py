...
_PLUS_MINUS_MARKER = re.compile(r"\\(?:pm|mp)(?![A-Za-z])|[±∓]")
...
def _set_alias(value: str) -> Optional[str]:
    """Return a canonical name for common empty/real-set spellings."""
    value = _strip_math_wrappers(value).replace("\\left", "").replace("\\right", "")
    for _ in range(4):
...
def _split_top_level_unions(value: str) -> List[str]:
    """Split ``\\cup``/``∪`` without splitting inside a component."""
    parts: List[str] = []
    start = 0
    depth = 0
    index = 0
    while index < len(value):
        ...
        is_latex_union = (
            depth == 0
            and value.startswith(r"\cup", index)
            and (index + 4 == len(value) or not value[index + 4].isalpha())
        )
        if depth == 0 and (char == "∪" or is_latex_union):
            part = value[start:index].strip()
            if not part:
                return []
            parts.append(part)
            index += 4 if is_latex_union else 1
            start = index
            continue
        index += 1
    if depth != 0:
...
def _parse_set_values(value: str):
    """Parse one finite-set item, expanding one ``\\pm`` or ``\\mp``."""
    markers = list(_PLUS_MINUS_MARKER.finditer(value))
    if len(markers) > 1:
        return None
    variants = [value]
    if markers:
        marker = markers[0]
        variants = [
            value[: marker.start()] + sign + value[marker.end() :]
            for sign in ("+", "-")
        ]
    parsed_values = []
    for variant in variants:
        source = _latex_to_sympy_source(variant)
        if source is None or re.search(r"(?<![<>=])(<=|>=|!=|=|<|>)(?![<>=])", source):
            return None
...
def _parse_finite_set_component(value: str):
    value = value.strip()
...
def _parse_interval_component(value: str):
    value = value.strip()
    if len(value) < 2 or value[0] not in "[(" or value[-1] not in ")]":
        return None
    if not _balanced_math_delimiters(value[1:-1]):
        return None
    parts = _split_top_level(value[1:-1])
    if len(parts) != 2:
        return None
    left_source = _latex_to_sympy_source(parts[0])
    right_source = _latex_to_sympy_source(parts[1])
    ...
    if left is None or right is None:
        return None
    return ("interval", left, right, value[0] == "[", value[-1] == "]")


def _parse_set_object(text: str):
    """Parse finite sets, intervals, their unions, and common aliases."""
    value = _strip_math_wrappers(text)
    value = value.replace("\\left", "").replace("\\right", "")
    alias = _set_alias(value)
    if alias == "empty":
        return ("set", tuple())
    if alias == "reals":
        return ("set", (("reals",),))

    pieces = _split_top_level_unions(value)
    if not pieces:
        return None
    has_union = len(pieces) > 1
    components = []
    for piece in pieces:
        piece_alias = _set_alias(piece)
        if piece_alias == "empty":
            continue
        if piece_alias == "reals":
            components.append(("reals",))
            continue
        component = _parse_finite_set_component(piece)
        if component is None:
            component = _parse_interval_component(piece)
        if component is None and not has_union and _PLUS_MINUS_MARKER.search(piece):
            values = _parse_set_values(piece)
            component = ("finite", values) if values is not None else None
        if component is None:
            return None
        components.append(component)
    return ("set", tuple(components))
...
def _parse_math_object(text: str):
    set_object = _parse_set_object(text)
...
def _parse_interval_object(text: str):
    """Compatibility wrapper for callers of the pre-v5 private helper."""
    parsed = _parse_set_object(text)
    if parsed is None or any(component[0] != "interval" for component in parsed[1]):
        return None
    return parsed
...
def _unordered_symbolic_equal(left_values, right_values) -> bool:
    """Compare small symbolic collections without making order significant."""
    left_unique = []
    for value in left_values:
        if not any(_sympy_equal(value, existing) for existing in left_unique):
            left_unique.append(value)
    right_unique = []
    for value in right_values:
        if not any(_sympy_equal(value, existing) for existing in right_unique):
            right_unique.append(value)
    if len(left_unique) != len(right_unique):
        return False

    matched_left_for_right = [-1] * len(right_unique)

    def augment(left_index: int, seen: set[int]) -> bool:
        for right_index, right_value in enumerate(right_unique):
            if right_index in seen or not _sympy_equal(left_unique[left_index], right_value):
                continue
            seen.add(right_index)
            previous = matched_left_for_right[right_index]
            if previous < 0 or augment(previous, seen):
                matched_left_for_right[right_index] = left_index
                return True
        return False

    return all(augment(index, set()) for index in range(len(left_unique)))


def _infinite_endpoint(value, *, positive: bool) -> bool:
    try:
        import sympy as sp

        return value == (sp.oo if positive else -sp.oo)
    except Exception:
        return False


def _canonical_set_parts(components):
    points = []
    intervals = []
    is_reals = False
    for component in components:
        if component[0] == "reals":
            is_reals = True
            continue
        if component[0] == "finite":
            points.extend(component[1])
            continue
        if component[0] != "interval":
            return None
        _, left, right, left_closed, right_closed = component
        if _infinite_endpoint(left, positive=False) and _infinite_endpoint(right, positive=True):
            is_reals = True
            continue
        if left_closed and right_closed and _sympy_equal(left, right):
            points.append(left)
            continue
        intervals.append((left, right, left_closed, right_closed))
    return is_reals, tuple(points), tuple(intervals)


def _interval_equivalent(left, right) -> bool:
    left_start, left_end, left_start_closed, left_end_closed = left
    right_start, right_end, right_start_closed, right_end_closed = right
    # A bracket at infinity carries no set-theoretic meaning, and generated
    # answers commonly vary between ``(-\\infty`` and ``[-\\infty``.
    if _infinite_endpoint(left_start, positive=False):
        left_start_closed = False
    if _infinite_endpoint(right_start, positive=False):
        right_start_closed = False
    if _infinite_endpoint(left_end, positive=True):
        left_end_closed = False
    if _infinite_endpoint(right_end, positive=True):
        right_end_closed = False
    return (
        left_start_closed == right_start_closed
        and left_end_closed == right_end_closed
        and _sympy_equal(left_start, right_start)
        and _sympy_equal(left_end, right_end)
    )


def _unordered_intervals_equal(left_intervals, right_intervals) -> bool:
    if len(left_intervals) != len(right_intervals):
        return False
    unmatched = list(right_intervals)
    for left in left_intervals:
        for index, right in enumerate(unmatched):
            if _interval_equivalent(left, right):
                unmatched.pop(index)
                break
        else:
            return False
...
def _set_equivalent(left_components, right_components) -> bool:
    left = _canonical_set_parts(left_components)
    right = _canonical_set_parts(right_components)
    if left is None or right is None:
        return False
    left_reals, left_points, left_intervals = left
    right_reals, right_points, right_intervals = right
    if left_reals or right_reals:
        return left_reals and right_reals
    return _unordered_symbolic_equal(left_points, right_points) and _unordered_intervals_equal(
        left_intervals, right_intervals
    )


def _finite_values_from_object(parsed):
    if parsed[0] == "expr":
        return (parsed[1],)
    ...
    if parsed[0] != "set":
        return None
    canonical = _canonical_set_parts(parsed[1])
    if canonical is None:
        return None
    is_reals, points, intervals = canonical
    return None if is_reals or intervals else points
...
def _symbolic_equivalent_uncached(prediction: str, gold_answer: str) -> bool:
    ...
    if predicted[0] != expected[0]:
        if "set" not in {predicted[0], expected[0]}:
            return False
        predicted_values = _finite_values_from_object(predicted)
        expected_values = _finite_values_from_object(expected)
        return (
            predicted_values is not None
            and expected_values is not None
            and _unordered_symbolic_equal(predicted_values, expected_values)
        )
    if predicted[0] == "expr":
    ...
    if predicted[0] == "set":
        return _set_equivalent(predicted[1], expected[1])
...
- Verifier turn:
    ...
    * if the prior answer seems right but the derivation is long or subtle,
      prefer handing off to the third agent for one more check.
- Final verifier:
...
- Collaboration guidance:
    * For nontrivial problems, prefer using at least two agents and often
      three when another independent check would reduce risk.
