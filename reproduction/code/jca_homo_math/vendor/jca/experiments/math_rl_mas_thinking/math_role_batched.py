...
def sanitize_protocol_step(step: base.MathStep) -> base.MathStep:
    normalized = base.MathStep(**sanitize_json_value(asdict(step)))
    if any(getattr(normalized, field) != getattr(step, field) for field in PROTOCOL_FIELDS):
        normalized.visible_output = canonical_protocol_visible_output(normalized)
    return normalized
...
def load_state(path: Path) -> Dict[str, Any]:
    ...
    finalize_exhausted_retries(state)
...
def finalize_exhausted_retries(state: Dict[str, Any]) -> int:
    if not bool(state["config"].get("retry_failed_groups", False)):
        return 0
    ...
    for item in state["items"]:
        if (
            item.get("status") == "retry"
            and _is_int(item.get("group_attempt"))
            and item["group_attempt"] == max_attempts
        ):
            item["status"] = "failed"
            finalized += 1
    return finalized
...
def terminate(
    ...
    *,
    ...
    if retry_failed_group and reason != "stop":
        exhausted = (
            group_retries is not None
            and int(item.get("group_attempt", 0)) >= int(group_retries)
        )
        item["status"] = "failed" if exhausted else "retry"
    else:
        item["status"] = "done"
        ...
        try:
            ...
            if not json_object_well_formed(visible):
                ...
                if schema_reason is not None:
                    ...
                    candidate = sanitize_protocol_step(candidate)
...
def _commit_accepted_step(
    ...
    if turn + 1 >= int(config["t_max"]):
        terminate(
            item,
            "truncated",
            retry_failed_group=retry_failed_group,
            group_retries=int(config["group_retries"]),
        )
        return item
...
def run_concurrent_pipeline(
    state: Dict[str, Any],
    *,
    bootstrap_caller: OpenAIChatLLMCaller,
    protocol_callers: Dict[str, Optional[OpenAIChatLLMCaller]],
    ...
    for agent in AGENT_IDS:
        capacity = int(concurrency_by_agent.get(agent, 0))
        if capacity < 0:
    ...
    def idle_overprovisioned_agents() -> list[str]:
        idle: list[str] = []
        for agent in AGENT_IDS:
            if int(concurrency_by_agent[agent]) <= base_capacity:
                continue
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        while True:
            ...
            if not draining_for_rebalance:
                for agent in AGENT_IDS:
                    capacity = int(concurrency_by_agent[agent])
                    if capacity <= 0:
                        continue
                    while (
            ...
            if not futures:
                ...
                if remaining:
                    ...
                    for index in remaining:
                        ...
                        if work is not None and int(concurrency_by_agent.get(work[1], 0)) <= 0:
            ...
            unavailable_ready_agents = sorted(
                {
                    ...
                    and int(concurrency_by_agent.get(work[1], 0)) <= 0
                }
            )
    ...
    state.setdefault("history", []).append({
        ...
        "advanced": advanced,
        "concurrency_by_agent": dict(concurrency_by_agent),
    ...
    if args.command == "run-concurrent":
        ...
        with StateJournal(args.state, args.journal_fsync_every) as journal:
            advanced = run_concurrent_pipeline(
                ...
                rebalance_after_advanced=args.rebalance_after_advanced,
