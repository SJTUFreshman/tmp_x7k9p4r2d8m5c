...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Allow a resumable checkpoint to be loaded with a different number "
            "of processes when the effective batch and schedule are unchanged."
        ),
    )
    p.add_argument(
        "--max-steps",
        type=int,
        default=0,
        help="Stop after this global optimizer step (debug/smoke testing only; 0 means no limit).",
    )
...
def resolve_resume_checkpoint(out_dir: Path, requested: Optional[str]) -> Optional[Path]:
    def complete(path: Path) -> bool:
        required = (
            ...
            path / "optimizer.bin",
            path / "custom_checkpoint_0.pkl",
            path / "policy_adapter" / "adapter_model.safetensors",
        )
...
def main() -> None:
    ...
    if resume_checkpoint is not None:
        ...
        logger.info("Resume:      no complete checkpoint found; starting from the SFT adapter")
    ...
    # DDP-friendly accelerator; picks up world_size from accelerate launch
    ...
    resume_schedule_step: Optional[int] = None
    stored_world_size = world_size
    stored_per_device_batch = args.per_device_batch_size
    if resume_checkpoint is not None:
        ...
        if stored_contract != checkpoint_contract:
            stored_contract = stored_contract or {}
            differing = sorted(
                key
                for key in set(checkpoint_contract) | set(stored_contract)
                if checkpoint_contract.get(key) != stored_contract.get(key)
            )
            ...
            if args.allow_world_size_change and set(differing).issubset(
                allowed_runtime_differences
            ):
                old_world_size = int(stored_contract.get("world_size", 0))
                old_grad_accum = int(
                    stored_contract.get("gradient_accumulation_steps", 0)
                )
                old_per_device_batch = int(
                    stored_contract.get("per_device_batch_size", 0)
                )
                old_effective_batch = (
                    old_world_size * old_per_device_batch * old_grad_accum
                )
                new_effective_batch = (
                    world_size
                ...
                stored_total_steps = int(stored_contract.get("total_steps", 0))
                stored_global_step = int(resume_payload.get("global_step", 0))
                if (
                    old_world_size <= 0
                    or old_per_device_batch <= 0
                    or old_grad_accum <= 0
                    or stored_total_steps <= 0
                    or total_steps < 1
                    or stored_global_step > total_steps
                ):
                    raise ValueError(
                        "checkpoint progress is incompatible with the current training schedule: "
                        f"checkpoint_step={stored_global_step}, "
                        f"current_total_steps={total_steps}"
                    )
                topology_changed = old_world_size != world_size
                schedule_changed = stored_total_steps != total_steps
                stored_world_size = old_world_size
                stored_per_device_batch = old_per_device_batch
                logger.warning(
                    "Resuming with changed process topology or schedule: "
                    f"checkpoint world_size={old_world_size}, "
                    f"gradient_accumulation_steps={old_grad_accum}; "
                    f"current world_size={world_size}, "
                    f"gradient_accumulation_steps={args.gradient_accumulation_steps}. "
                    f"effective_batch={old_effective_batch}->{new_effective_batch}, "
                    f"total_steps={stored_total_steps}->{total_steps}."
                )
            else:
                raise ValueError(
        ...
        if (topology_changed or schedule_changed) and resume_epoch != 0:
            raise ValueError(
                "changed-topology resume is supported only within the first epoch"
            )
        if topology_changed or schedule_changed:
            # ``next_micro_idx`` is local to each rank.  When the process
            # count changes, map it through the number of global records
            # already consumed so optimizer-step and data progress remain
            # aligned (the effective batch is checked above).
            consumed_records = (
                resume_micro_idx * stored_world_size * stored_per_device_batch
            )
            new_micro_width = world_size * args.per_device_batch_size
            mapped_micro_idx = (consumed_records + new_micro_width - 1) // new_micro_width
            accumulation = args.gradient_accumulation_steps
            if mapped_micro_idx % accumulation:
                mapped_micro_idx += accumulation - (mapped_micro_idx % accumulation)
            mapped_records = mapped_micro_idx * new_micro_width
            resume_schedule_step = mapped_records // new_effective_batch
            if resume_schedule_step > total_steps:
                raise ValueError(
                    "mapped checkpoint progress exceeds the current schedule: "
                    f"mapped_step={resume_schedule_step}, total_steps={total_steps}"
                )
            logger.info(
                f"Remapped checkpoint micro-batch offset "
                f"{resume_micro_idx} -> {mapped_micro_idx} for world_size "
                f"{stored_world_size} -> {world_size} "
                f"(global_records={consumed_records}->{mapped_records})"
            )
            resume_micro_idx = mapped_micro_idx
            # Accelerate restores the old local micro-batch counter.  Reset it
            # to the mapped counter so the new accumulation window starts at a
            # clean optimizer boundary and future checkpoints record progress
            # in the new topology.
            accelerator.step = resume_micro_idx
        logger.info(
            f"Resumed checkpoint at global_step={global_step}, "
            f"next_epoch={resume_epoch}, next_micro_idx={resume_micro_idx}"
        )
        if resume_schedule_step is not None and resume_schedule_step != global_step:
            global_step = resume_schedule_step
            scheduler.step(global_step)
            logger.info(
                f"Adjusted resumed optimizer-step index to {global_step} "
                "for the new effective batch"
            )
    ...
    for epoch in range(epochs):
        ...
        for micro_idx, step_idx in enumerate(range(0, len(my_records), batch_size)):
            ...
            local_dummy = tensors is None
            dummy_batch = bool(
                accelerator.gather(
                    torch.tensor([int(local_dummy)], device=accelerator.device)
                ).any().item()
            )
            if not dummy_batch:
        ...
        # Epoch boundaries are the useful evaluation/resume artifacts.  Keep
        # periodic saves optional so a full run does not accumulate dense,
        # multi-gigabyte optimizer snapshots between epochs.
        if global_step > last_checkpoint_step:
            save_training_checkpoint(epoch + 1, 0)
            last_checkpoint_step = global_step

    if args.max_steps:
        logger.info(f"Reached debug max_steps={args.max_steps}; no final adapter was written")
        accelerator.wait_for_everyone()
        return
