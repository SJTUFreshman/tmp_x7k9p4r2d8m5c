...
def normalize_numeric_answers(obj: dict[str, Any]) -> dict[str, Any]:
    """Preserve numeric answer scalars as their canonical JSON text."""
    normalized = dict(obj)
    for field in ("tentative_answer", "confirmed_answer"):
        value = normalized.get(field)
        if isinstance(value, (int, float)) and not isinstance(value, bool):
            normalized[field] = json.dumps(value, ensure_ascii=False, allow_nan=False)
    return normalized
...
def parse_protocol(
    ...
    *,
    ...
    coerce_numeric_answers: bool = False,
    ...
    if coerce_numeric_answers:
        try:
            obj = normalize_numeric_answers(obj)
        except (TypeError, ValueError):
            return None, "protocol answer contains a non-finite number"
