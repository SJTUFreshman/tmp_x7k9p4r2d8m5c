...
def run_synchronous(
    ...
    *,
    ...
    for turn in range(t_max):
        ...
        for agent in AGENT_IDS:
            raw, parsed, error, finish_reason = outcomes[agent]
            step = AgentTurn(
                agent=agent,
                turn=turn,
                prompt_messages=prompts[agent],
                response=raw,
                parsed=parsed,
...
def run_sequential(
    ...
    *,
    ...
    for turn in range(t_max):
        ...
        raw, parsed, error, finish_reason = _call_with_retry(
            callers[active],
            task,
            snapshot,
            agent=active,
