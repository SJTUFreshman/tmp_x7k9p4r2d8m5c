...
def _wire_messages(messages: list[dict[str, str]]) -> list[dict[str, str]]:
    """Keep valid Unicode intact; escape only characters UTF-8 cannot carry.

    Model JSON can contain isolated surrogate escapes.  Preserve the original
    trajectory strings, but make their subsequent prompt representation safe
    for the server tokenizer.
    """
    return [
        {key: (value.encode("utf-8", "backslashreplace").decode("utf-8")
