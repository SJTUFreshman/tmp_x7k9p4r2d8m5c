...
class ConiferTask(TaskAdapter):
    ...
    def aggregate(self, traj: JointTrajectory) -> str | None:
        """Take A3's final answer; majority voting on free-form prose is meaningless."""
        if traj.joint_mode == "sequential":
            return super().aggregate(traj)
        if not traj.turns:
            return None
        ...
        # A3 is the integrator and has seen A1/A2's drafts; fall back down the
