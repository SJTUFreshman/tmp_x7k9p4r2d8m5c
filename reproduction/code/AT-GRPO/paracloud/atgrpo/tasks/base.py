...
class TaskAdapter(abc.ABC):
    ...
    def parse(
        ...
        return parse_protocol(
            raw,
            active_agent=agent,
            allow_aliases=self.allow_field_aliases,
            coerce_numeric_answers=self.coerce_numeric_answers,
        )
    ...
    def score_answers(
        ...
        AT-GRPO scores every tree candidate by its episode outcome, so the same
        problem appears once per candidate. Batched so MultiPL-E amortizes one
        docker invocation over the whole iteration.
        """
