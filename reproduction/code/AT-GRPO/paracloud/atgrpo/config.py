...
class Config:
    ...
    def validate(self) -> None:
        ...
        if self.serving.gpu_plan not in {
            "split31", "split44", "split53", "timeshare", "restart"
        }:
            errors.append(
                "serving.gpu_plan must be split31|split44|split53|timeshare|restart"
            )
        try:
            serve_assignments = [
                gpu
            ...
            if duplicate_train and self.serving.gpu_plan != "split31":
                errors.append(
            ...
            if self.serving.gpu_plan == "split31" and (
                any(len(gpu_ids(self.serving.serve_gpus[agent])) != 1 for agent in AGENTS)
                or any(len(gpu_ids(self.serving.train_gpus[agent])) != 1 for agent in AGENTS)
                or len(serve) != 3
                or len(train) != 1
            ):
                errors.append(
                    "serving.gpu_plan='split31' requires three distinct single-GPU "
                    "servers and all three trainers sharing one separate GPU"
                )
