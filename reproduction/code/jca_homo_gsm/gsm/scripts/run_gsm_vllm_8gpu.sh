#!/usr/bin/env bash
# GSM-HARD evaluation launcher — 8 GPU vLLM.
#
...
SINGLE_AGENT_RUNNER="${SINGLE_AGENT_RUNNER:-gsm/scripts/run_single_agent.py}"
...
case "$MODE" in
  ...
  RUN_CMD=(
      "$PYTHON_BIN" -u "$SINGLE_AGENT_RUNNER"
      --data-path "$DATA_PATH"
    ...
    if [[ "$SINGLE_AGENT_RUNNER" == *"run_single_agent_corrected"* ]]; then
      RUN_CMD+=(--max-model-len "$MAX_MODEL_LEN")
    else
      RUN_CMD+=(--log-raw-chars "$LOG_RAW_CHARS")
    fi
  ...
  zero_shot_mas|sft_lora_mas)
    ...
    RUN_CMD=(
      "$PYTHON_BIN" -u gsm/scripts/run_mas.py
      --data-path "$DATA_PATH"
      --start "$START" --limit "$LIMIT"
      --t-max "$T_MAX"
    ...
    if [[ "$ENFORCE_COLLABORATION_POLICY" == "0" ]]; then
      RUN_CMD+=(--no-enforce-collaboration-policy)
    fi
    if [[ "$SFT_WARMUP_PROMPT" == "1" ]]; then
...
if [[ -n "$GENERATION_SEED" ]]; then
  RUN_CMD+=(--generation-seed "$GENERATION_SEED")
fi
