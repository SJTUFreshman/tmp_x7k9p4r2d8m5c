...
EVAL_CMD=(
  ...
  "EVAL_BOOTSTRAP_HANDOFF_TARGET=$EVAL_BOOTSTRAP_HANDOFF_TARGET"
  "EVAL_PRESERVE_REASONABLE_INCUMBENT=$EVAL_PRESERVE_REASONABLE_INCUMBENT"
  "EVAL_LOCK_UPSTREAM_ANSWER_AGENTS=$EVAL_LOCK_UPSTREAM_ANSWER_AGENTS"
  ...
  "EVAL_SCHEDULER_MODE=adaptive_partition" "EVAL_ADAPTIVE_LAYOUT=1"
  "EVAL_ADAPTIVE_ALLOW_ZERO=0" "EVAL_REBALANCE_AFTER_ADVANCED=64"
  "EVAL_MAX_ADVANCED_PER_PASS=0" "EVAL_MAX_INFLIGHT=128"
  "EVAL_LIVE_ENDPOINT_REBALANCE=1" "EVAL_LIVE_REBALANCE_POLL_SECONDS=2"
  "EVAL_LIVE_RETIRE_TIMEOUT=60" "EVAL_LIVE_REBALANCE_MIN_ADVANCED=64"
  "EVAL_SHARED_GPU_IDS=$GPU_IDS" "EVAL_SHARED_DP=$NUM_GPUS"
  "EVAL_A1_GPU_IDS=0,1" "EVAL_A2_GPU_IDS=2,3" "EVAL_A3_GPU_IDS=4,5,6,7"
  "EVAL_A1_DP=2" "EVAL_A2_DP=2" "EVAL_A3_DP=4"
  "EVAL_A1_MAX_CONCURRENCY=32" "EVAL_A2_MAX_CONCURRENCY=32"
  "EVAL_A3_MAX_CONCURRENCY=64" "EVAL_PROTOCOL_THINKING_MAX_TOKENS=2048"
...
"$PYTHON_BIN" - "$variant_manifest" "$variant" "$EVAL_SHARD_FILE" \
  ...
  "$adapter_a1" "$adapter_a2" "$adapter_a3" "$EVAL_LIMIT" \
  "$EVAL_BOOTSTRAP_AGENT" "$EVAL_BOOTSTRAP_HANDOFF_TARGET" \
  "$EVAL_PRESERVE_REASONABLE_INCUMBENT" \
  "$EVAL_LOCK_UPSTREAM_ANSWER_AGENTS" <<'PY'
import hashlib
import json
import os
import sys
...
value = {
    ...
    "generation": {
        ...
        "preserve_reasonable_incumbent": bool(int(preserve_reasonable_incumbent)),
        "lock_upstream_answer_agents": [
            value.strip()
            for value in lock_upstream_answer_agents.split(",")
            if value.strip()
        ],
        "seen_agent_handoff_policy": "allowed",
    },
}
