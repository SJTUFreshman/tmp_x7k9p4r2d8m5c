...
validate_static_config() {
  ...
  [[ "$EVAL_BOOTSTRAP_HANDOFF_TARGET" =~ ^A[123]$ ]] || \
    fatal "EVAL_BOOTSTRAP_HANDOFF_TARGET must be A1, A2, or A3"
  [[ "$EVAL_LOCK_UPSTREAM_ANSWER_AGENTS" =~ ^(A[123](,A[123])*)?$ ]] || \
    fatal "EVAL_LOCK_UPSTREAM_ANSWER_AGENTS must be a comma-separated list of A1/A2/A3"
  [[ "$EVAL_VARIANT_NAME" =~ ^[A-Za-z0-9._-]+$ ]] || \
    fatal "EVAL_VARIANT_NAME contains unsupported characters"
