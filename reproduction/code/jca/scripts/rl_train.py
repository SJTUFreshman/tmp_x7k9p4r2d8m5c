...
def _infer_response_thinking_flag(
    ...
    if not isinstance(response, str) or not response.strip():
        return None
    if _protocol_json_well_formed(response):
        return False
...
def _training_response_for_mode(row: Dict[str, Any], mode: bool, line_number: int) -> str:
    ...
    is_plain_json = _protocol_json_well_formed(response)
    if is_plain_json and not mode:
        return response
    matches = [] if is_plain_json else list(_THINK_BLOCK_RE.finditer(response))
    has_tag = not is_plain_json and (
        bool(matches) or bool(re.search(r"<\/?think\b", response, flags=re.IGNORECASE))
    )
    ...
    if mode:
        if not has_nonempty:
            ...
            if thinking:
                ...
                visible_response = response.lstrip() if is_plain_json else _THINK_BLOCK_RE.sub("", response).lstrip()
                response = f"<think>\n{thinking}\n</think>\n{visible_response}"
            else:
                raise ValueError(
