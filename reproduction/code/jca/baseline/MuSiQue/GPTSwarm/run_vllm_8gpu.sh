#!/usr/bin/env bash
# 8-GPU heterogeneous GPTSwarm-Fixed launcher: start three base-model
# vLLM servers, then route node types by model capacity and report EM/F1.
#
# GPU layout:
#   IO nodes          -> A1 Qwen3-1.7B -> GPUs 0,1     TP=2
#   CoT nodes         -> A2 Qwen3-4B   -> GPUs 2,3     TP=2
#   Debate/Aggregator -> A3 Qwen3-8B   -> GPUs 4,5,6,7 TP=4
#
# Smoke test (10 problems):
#   LIMIT=10 bash baseline/MuSiQue/GPTSwarm/run_vllm_8gpu.sh
#
# Full dev run:
#   LIMIT=2417 bash baseline/MuSiQue/GPTSwarm/run_vllm_8gpu.sh
...
OUTPUT_DIR="${OUTPUT_DIR:-baseline/MuSiQue/GPTSwarm/outputs}"
LOG_DIR="${LOG_DIR:-baseline/MuSiQue/GPTSwarm/logs}"

RUNNER_PATH="${RUNNER_PATH:-baseline/MuSiQue/GPTSwarm/run_gptswarm.py}"
NODES_PATH="${NODES_PATH:-baseline/MuSiQue/GPTSwarm/nodes.py}"
SWARM_PATH="${SWARM_PATH:-baseline/MuSiQue/GPTSwarm/swarm.py}"
PROMPT_IO_PATH="${PROMPT_IO_PATH:-baseline/MuSiQue/GPTSwarm/prompts/node_io.md}"
PROMPT_COT_PATH="${PROMPT_COT_PATH:-baseline/MuSiQue/GPTSwarm/prompts/node_cot.md}"
PROMPT_DEBATE_PATH="${PROMPT_DEBATE_PATH:-baseline/MuSiQue/GPTSwarm/prompts/node_debate.md}"
PROMPT_AGGREGATOR_PATH="${PROMPT_AGGREGATOR_PATH:-baseline/MuSiQue/GPTSwarm/prompts/node_aggregator.md}"
...
RUN_ID="${RUN_ID:-$(date +%Y%m%d_%H%M%S)_gptswarm_${SPLIT}_start${START}_n${LIMIT}}"
...
start_base_server() {
  ...
  echo "  log:    $log_path"
  setsid env CUDA_VISIBLE_DEVICES="$gpus" LD_LIBRARY_PATH="$VLLM_LD_LIBRARY_PATH" \
...
echo "================ GPTSwarm-Fixed Heterogeneous 8-GPU Run ================"
...
echo "SPLIT=$SPLIT START=$START LIMIT=$LIMIT MAX_CONCURRENCY=$MAX_CONCURRENCY"
echo "ROUTING_POLICY=$ROUTING_POLICY"
echo "TEMPERATURE=$TEMPERATURE TOP_P=$TOP_P MAX_NEW_TOKENS=$MAX_NEW_TOKENS"
echo "TORCH_DTYPE=$TORCH_DTYPE ENABLE_THINKING=$ENABLE_THINKING"
echo "A1 server: model=$MODEL_A1 served=$SERVED_A1 port=$A1_PORT gpus=$A1_GPUS tp=$A1_TP"
echo "A2 server: model=$MODEL_A2 served=$SERVED_A2 port=$A2_PORT gpus=$A2_GPUS tp=$A2_TP"
echo "A3 server: model=$MODEL_A3 served=$SERVED_A3 port=$A3_PORT gpus=$A3_GPUS tp=$A3_TP"
...
require_file "GPTSwarm runner" "$RUNNER_PATH"
require_file "nodes module" "$NODES_PATH"
require_file "swarm module" "$SWARM_PATH"
require_file "IO prompt" "$PROMPT_IO_PATH"
require_file "CoT prompt" "$PROMPT_COT_PATH"
require_file "Debate prompt" "$PROMPT_DEBATE_PATH"
require_file "Aggregator prompt" "$PROMPT_AGGREGATOR_PATH"
require_file "vLLM tokenizer compatibility shim" "$VLLM_PYTHON_COMPAT_DIR/sitecustomize.py"

"$PYTHON_BIN" --version
env PYTHONPATH="${VLLM_PYTHON_COMPAT_DIR}${PYTHONPATH:+:${PYTHONPATH}}" "$PYTHON_BIN" - <<'PY'
...
if [[ "$USE_EXISTING_SERVERS" == "1" ]]; then
  check_existing_server "$SERVED_A1" "$A1_PORT" || {
    echo "ERROR: USE_EXISTING_SERVERS=1 but A1 not ready on $A1_PORT." >&2
    exit 1
  }
  check_existing_server "$SERVED_A2" "$A2_PORT" || {
    echo "ERROR: USE_EXISTING_SERVERS=1 but A2 not ready on $A2_PORT." >&2
    exit 1
  }
  check_existing_server "$SERVED_A3" "$A3_PORT" || {
    echo "ERROR: USE_EXISTING_SERVERS=1 but A3 not ready on $A3_PORT." >&2
    exit 1
  }
else
