...
validate_inputs() {
  ...
  for value_name in ENFORCE_COLLABORATION_POLICY SFT_WARMUP_PROMPT \
    ...
    GLOBAL_TURN_BATCHING ENABLE_THINKING USE_LORA; do
...
start_server() {
  ...
  if [[ "$USE_LORA" == "1" ]]; then
    ...
    command+=(--served-model-name "$agent")
  fi
  ...
  for (( attempt = 1; attempt <= START_RETRIES; attempt++ )); do
    ...
    local mode_label="zero-shot"
    [[ "$USE_LORA" == "1" ]] && mode_label="LoRA"
    echo "Starting $agent $mode_label: GPUs=$GPU_IDS TP=$TENSOR_PARALLEL_SIZE DP=$DATA_PARALLEL_SIZE (attempt $attempt/$START_RETRIES)"
    echo "  model:   $ACTIVE_MODEL"
    [[ "$USE_LORA" == "1" ]] && echo "  adapter: $ACTIVE_ADAPTER"
