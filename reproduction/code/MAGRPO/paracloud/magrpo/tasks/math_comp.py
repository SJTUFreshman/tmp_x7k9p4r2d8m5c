...
class MathTask(TaskAdapter):
    ...
    def _score(self, problem: Problem, answer: str | None) -> tuple[float, dict]:
        if not answer:
            return 0.0, {
                ...
                "em": 0.0,
                "f1": 0.0,
                "f1_numeric_eligible": False,
                "f1_numeric": None,
                "grader": MATH_EVAL_VERSION,
                "f1_grader": MATH_SOFT_F1_VERSION,
            }
        ...
        f1, numeric_eligible, numeric_f1 = math_soft_f1(extracted, gold)
        return float(em), {
            "em": float(em),
            "f1": float(f1),
            "f1_numeric_eligible": bool(numeric_eligible),
    ...
    def eval_metric(self, results: Sequence[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "f1_numeric": fmean(
                float(r.detail["f1_numeric"])
                for r in results
                if r.detail.get("f1_numeric") is not None
            ) if any(r.detail.get("f1_numeric") is not None for r in results) else None,
            "n": len(results),
